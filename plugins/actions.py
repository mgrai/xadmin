...
class ActionPlugin(BaseAdminPlugin):
    ...
    def init_request(self, *args, **kwargs):
        self.actionsX = self.get_actions()
        return bool(self.actionsX)
    ...
    def get_context(self, context):
        if self.actionsX and self.admin_view.result_count:
            av = self.admin_view
    ...
    def post_response(self, response, *args, **kwargs):
        ...
        if self.actionsX and 'action' in request.POST:
            action = request.POST['action']

            if action not in self.actionsX:
                msg = _("Items must be selected in order to perform "
                        "actions on them. No items have been changed.")
                av.message_user(msg)
            else:
                ac, name, description, icon = self.actionsX[action]
    ...
    def get_actions(self):
        ...
        for klass in self.admin_view.__class__.mro()[::-1]:
            ...
            if not isinstance(class_actions, (list, tuple)):
                actions.extend([self.get_action(action) for action in self.actions])
    ...
    def get_action_choices(self):
        ...
        """
        choices = []
        for ac, name, description, icon in self.actionsX.itervalues():
